...
| `rv_cd`    | `cd/rv_cd.h`       | **C**ontroller **D**isk (drive)     | `asset_open` (name → handle) / `asset_size` / `asset_read` into the game's buffer, `resource_addr`/`_palette_addr`/`_width`/`_height` (kind + name → VRAM address; `rv_cd_resource_kind` names what is being asked for, one enumerator today; the drive makes it resident on first ask and frees it itself when the disc unloads) |
