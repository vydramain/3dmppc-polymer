...
// Colour arithmetic for discs. rv_pdk::rv_color (pdk/cv/rv_vertex.hpp) is three
...
//
// The working type is float in [0, 1] per channel; rv_pdk::rv_color is the packed
// form handed to the console. Conversions saturate rather than wrap: an
...
inline rv_pdk::rv_color rv_color_make(float r, float g, float b) {
    return rv_pdk::rv_color{rv_color_channel(r), rv_color_channel(g), rv_color_channel(b)};
}

inline rv_vec3 rv_color_to_vec3(rv_pdk::rv_color c) {
    return rv_vec3{static_cast<float>(c.r) / 255.0f, static_cast<float>(c.g) / 255.0f,
                   static_cast<float>(c.b) / 255.0f};
}

inline rv_pdk::rv_color rv_color_from_vec3(rv_vec3 v) { return rv_color_make(v.x, v.y, v.z); }
...
inline rv_pdk::rv_color rv_hsv_to_rgb(float h, float s, float v) {
...
inline rv_pdk::rv_color rv_color_lerp(rv_pdk::rv_color a, rv_pdk::rv_color b, float t) {
    if (!(t > 0.0f)) return a;
    if (t >= 1.0f) return b;
...
inline rv_pdk::rv_color rv_color_scale(rv_pdk::rv_color c, float k) {
    return rv_color_from_vec3(rv_color_to_vec3(c) * k);
}

// Channel-wise product, the "modulate" of a texture against a vertex colour.
inline rv_pdk::rv_color rv_color_modulate(rv_pdk::rv_color a, rv_pdk::rv_color b) {
    const rv_vec3 va = rv_color_to_vec3(a);
    const rv_vec3 vb = rv_color_to_vec3(b);
...
inline rv_pdk::rv_color rv_color_add(rv_pdk::rv_color a, rv_pdk::rv_color b) {
...
inline rv_pdk::rv_color rv_shade_lambert(rv_pdk::rv_color base, rv_vec3 normal, rv_vec3 to_light,
                                         float ambient) {
