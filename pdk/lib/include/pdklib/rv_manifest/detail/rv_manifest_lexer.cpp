...
static rv_manifest_token token_of(tk kind, int line, std::string text = std::string())
{
	rv_manifest_token token;
	token.kind = kind;
	token.line = line;
...
static rv_manifest_token invalid(int line, std::string message)
{
	return token_of(tk::INVALID, line, std::move(message));
}
bool rv_manifest_lexer::eof() const
{
	return pos_ >= text_.size();
}

char rv_manifest_lexer::peek(std::size_t ahead) const
{
	return pos_ + ahead < text_.size() ? text_[pos_ + ahead] : '\0';
}

char rv_manifest_lexer::get()
{
...
void rv_manifest_lexer::skip_inline()
{
...
void rv_manifest_lexer::skip_comment()
{
	while (!eof() && peek() != '\n') {
		get();
	}
...
rv_manifest_token rv_manifest_lexer::next()
{
	for (;;) {
		skip_inline();
		if (eof()) {
		...
		if (rv_manifest_is_digit(c) || ((c == '-' || c == '+') && rv_manifest_is_digit(peek(1)))) {
			return lex_integer();
		}
		if (rv_manifest_is_ident(c)) {
			return lex_ident();
		}
		get();
...
rv_manifest_token rv_manifest_lexer::lex_ident()
{
	const int line = line_;
	std::string word;
...
rv_manifest_token rv_manifest_lexer::lex_string()
{
	const int line = line_;
	get(); // opening quote
	std::string out;
...
rv_manifest_token rv_manifest_lexer::lex_integer()
{
	const int line = line_;
	...
	rv_manifest_token token = token_of(tk::INTEGER, line);
	token.num = static_cast<int64_t>(parsed);
	return token;
}

std::vector<rv_manifest_token> rv_manifest_lex(const std::string &text)
{
	rv_manifest_lexer lexer(text);
	std::vector<rv_manifest_token> tokens;
	for (;;) {
		rv_manifest_token token = lexer.next();
