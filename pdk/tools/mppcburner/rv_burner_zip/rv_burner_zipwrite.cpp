...
// consults the UTF-8 flag above, so a host byte of 0 turns a non-ASCII name
// (say "cafe.txt" with an acute e) into mojibake on extraction even though
// bit 11 is set. Verified against unzip 6.0.
...
static void put_bytes(std::vector<uint8_t> &out, const std::string &s)
{
...
rv_zipwriter::rv_zipwriter(const std::string &path)
    : impl_(new rv_zipwriter_impl())
{
    impl_->path = path;
    ...
    std::string &error)
{
    ...
    put_le_u16(header, static_cast<uint16_t>(name.size()));
...
bool rv_zipwriter::finish(std::string &error)
{
    ...
    put_le_u16(eocd, count); // entries in total
    put_le_u32(eocd, static_cast<uint32_t>(directory.size()));
