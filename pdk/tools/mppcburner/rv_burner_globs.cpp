	...
	std::vector<std::string> &out)
{
	...
	if (!rv_pdktools::has_wildcard(component)) {
		std::error_code ec;
	...
	for (const fs::directory_entry &child : sorted_children(here)) {
		...
		if (!rv_pdktools::wildcard_match(component, name)) {
			continue;
		}
		if (child.is_symlink()) {
			continue;
...
bool rv_pdktools::rv_glob_expand(const fs::path &root, const std::vector<std::string> &patterns,
