...
// Ревизия: stage 10 — разбор, рендер и валидация манифеста disc.toml.
// ──────────────────────────────────────────────────────────────────────────────
//
// A hand-written recursive-descent reader for the TOML SUBSET rv_manifest.hpp
// describes: section headers, `key = "string"`, `key = 42`, `key = ["a", "b"]`
// (possibly spread over several lines), `#` comments and blank lines. Nothing
// else — no tables-in-tables, no booleans, no floats, no dates, no bare keys on
// the right-hand side.
//
// WHY EVERY ERROR CARRIES A LINE NUMBER. This file is edited by a human in a
// text editor, and the parser is the only thing that ever reads it back. A
// parser that shrugs at what it does not understand — skipping an unknown key,
// tolerating a stray token — turns a typo into a disc that builds and is quietly
// missing a source file, and the developer finds out at run time, in a linker
// error or a black screen, with nothing pointing back at the manifest. So every
// failure path here refuses the whole file and names the line: `sources` typed
// as `source` costs a five-second fix instead of an afternoon. Errors are
// reported at the line the offending CONSTRUCT STARTED on (the opening quote of
// a string, the opening bracket of an array), because that is the line the
// author has to go and look at, not the line where the parser finally gave up.
...
constexpr std::string_view kBudgetKeys[] = {"texture_max_width", "texture_max_height",
                                            "video_memory_size"};
...
constexpr section_spec kSections[] = {
    {"disc", kDiscKeys, std::size(kDiscKeys)},
    {"build", kBuildKeys, std::size(kBuildKeys)},
    {"assets", kAssetsKeys, std::size(kAssetsKeys)},
    {"textures", kTexturesKeys, std::size(kTexturesKeys)},
    {"budget", kBudgetKeys, std::size(kBudgetKeys)},
};
...
constexpr std::string_view kTextureFormats[] = {"idx4", "idx8", "direct15"};

const section_spec* find_section(std::string_view name) {
    for (const section_spec& s : kSections) {
...
std::string suggest(std::string_view word, const std::string_view* candidates, std::size_t count) {
    ...
    if (best.empty() || best_distance > limit) return std::string();
    return " (did you mean '" + std::string(best) + "'?)";
}

std::string suggest_section(std::string_view word) {
    std::vector<std::string_view> names;
    names.reserve(std::size(kSections));
    for (const section_spec& s : kSections) names.push_back(s.name);
...
enum class value_kind { string, integer, array };
...
class manifest_parser {
   public:
    manifest_parser(const std::string& text, rv_manifest& out) : text_(text), manifest_(out) {}
    ...
    bool run(std::string& error) {
        ...
        for (;;) {
            ...
            const char c = peek();
            if (c == '[') {
                if (!parse_section()) break;
            } else if (is_ident(c)) {
                if (!parse_assignment()) break;
            } else {
                fail(line_,
   ...
   private:
    ...
    bool parse_section() {
        ...
        skip_inline();
        std::string name;
        while (!eof() && is_ident(peek())) name += get();
        skip_inline();
        if (name.empty()) return fail(line, "empty section header");
        if (eof() || peek() != ']') {
            return fail(line, "section header '[" + name + "' is missing its closing ']'");
        }
        get();  // ']'
        if (find_section(name) == nullptr) {
            return fail(line, "unknown section '[" + name + "]'" + suggest_section(name));
        }
    ...
    bool parse_assignment() {
        const int line = line_;
        std::string key;
        while (!eof() && is_ident(peek())) key += get();
        skip_inline();
        if (eof() || peek() != '=') {
        ...
        if (!assign(key, value, line)) return false;
    ...
    bool parse_value(mvalue& out) {
        ...
        if (eof() || c == '\n' || c == '#') return fail(out.line, "missing value after '='");
        if (c == '\'') {
        ...
        return fail(out.line, "unsupported value '" + word +
                                  "' — expected a quoted string, an integer or an array of "
                                  "quoted strings");
    }
        ...
        for (;;) {
            ...
            switch (e) {
                ...
                case 'r':
                    ...
                    break;
                default:
                    return fail(escape_line, std::string("unknown escape '\\") + e +
                                                 "' in string — only \\\" \\\\ \\n \\t \\r are "
                                                 "recognised");
            }
        }
    ...
    bool parse_array(std::vector<std::string>& out) {
        ...
        for (;;) {
            ...
            std::string element;
            if (!parse_string(element)) return false;
    ...
    bool parse_integer(int64_t& out) {
        ...
        if (peek() == '-' || peek() == '+') get();
        if (eof() || !is_digit(peek())) return fail(start, "expected a decimal integer");
        while (!eof() && is_digit(peek())) get();
        ...
        if (!eof() && is_ident(peek())) {
            ...
            while (!eof() && is_ident(peek())) word += get();
    ...
    bool wrong_type(const std::string& key, const mvalue& v, value_kind want) {
        return fail(v.line, "key '" + key + "' in [" + section_ + "] takes " +
                                std::string(kind_name(want)) + ", not " +
    ...
    bool assign(const std::string& key, const mvalue& value, int key_line) {
        ...
        if (spec == nullptr) return fail(key_line, "internal error: unknown current section");

        bool known = false;
        for (std::size_t i = 0; i < spec->key_count; ++i) {
            if (spec->keys[i] == key) known = true;
        }
        if (!known) {
            return fail(key_line, "unknown key '" + key + "' in [" + section_ + "]" +
                                      suggest(key, spec->keys, spec->key_count));
        }
        ...
        if (section_ == "build" || section_ == "assets" ||
            ...
            if (value.kind != value_kind::array) return wrong_type(key, value, value_kind::array);
            if (key == "sources") {
        ...
        if (section_ == "textures") {  // key == "format"
            if (value.kind != value_kind::string) return wrong_type(key, value, value_kind::string);
            manifest_.textures.format = value.str;
            return true;
        }

        // [budget]
        if (value.kind != value_kind::integer) return wrong_type(key, value, value_kind::integer);
...
void render_array(std::ostringstream& out, const std::string& key,
    ...
    out << key << " = [\n";
    for (const std::string& v : values) out << "    " << quote(v) << ",\n";
    out << "]\n";
}
...
bool rv_manifest_validate(const rv_manifest& manifest, std::string& error) {
    ...
    for (std::string_view f : kTextureFormats) {
        if (f == manifest.textures.format) format_known = true;
    }
    if (!format_known) {
        std::string known;
        for (std::size_t i = 0; i < std::size(kTextureFormats); ++i) {
            if (i != 0) known += ", ";
            known += std::string(kTextureFormats[i]);
        }
