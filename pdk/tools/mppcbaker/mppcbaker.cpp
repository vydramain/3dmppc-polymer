...
// background. rv_texel_opaque is that fix, and it is applied at every point
// below where an opaque colour becomes a 16-bit word.
...
uint8_t channel_of(const rv_color5 &c, int axis)
{
...
struct options {
    ...
    rv_color key;         // the colour that becomes a hole
};
...
// Everything the program does. main() below holds no logic of its own: it has to
// live in the GLOBAL namespace because the language says so, and that is the only
// reason it is outside rv_pdktools.
int run(int argc, char **argv)
{
    ...
    for (size_t i = 0; i < texel_count; ++i) {
        ...
        src[i].color = rv_texel_quantize(c);
    ...
    if (opt.format == 15) {
        ...
        std::vector<color_bin> bins;
        {
            ...
            for (uint32_t code = 0; code < counts.size(); ++code) {
                if (counts[code] != 0) {
                    bins.push_back(
                        color_bin{ rv_texel_unpack(static_cast<uint16_t>(code)), counts[code] });
                }
            }
        }
        if (bins.empty()) {
        ...
        for (const rv_color5 &c : palette) {
            put_u16(file, rv_texel_opaque(rv_texel_pack(c)));
        }
        ...
        if (opt.format == 8) {
            ...
            for (int y = 0; y < height; ++y) {
                ...
                for (size_t x = 0; x < static_cast<size_t>(width); x += 2) {
                    ...
                    const uint8_t high = (x + 1 < static_cast<size_t>(width)) ? static_cast<uint8_t>(indices[row + x + 1] & 0x0F) : uint8_t{ 0 };
    ...
    std::printf("mppcbaker: %s -> %s (%dx%d, %s, %zu bytes)\n", opt.input.c_str(),
        opt.output.c_str(), width, height,
        opt.format == 4 ? "idx4" : (opt.format == 8 ? "idx8" : "direct15"), file.size());
    return 0;
}
