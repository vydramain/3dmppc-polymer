...
Версия документа: 1.3. Дата: 2026-09-25. UI-06: интерфейс - растровый шрифт 5x7 из pdklib, код и логи - растровый 6x11 редактора (ADR-0004); NFR-03: масштаб только целый.
...
- **TXT-04 MUST:** tabs/spaces, tab width, wrap, ruler, relative/absolute line numbers и indent guides задаются по языку. Начальный профиль C++: настоящие tabs шириной 4, ruler 128, без принудительного wrap; для остальных языков профиль настраивается отдельно.
...
- **UI-06 MUST:** шрифт UI и моноширинный шрифт кода/терминала - отдельные настройки на отдельных файлах. UI - растровый шрифт 5x7 в клетке 8x8 из pdklib (`rv_font`, с блоком кириллицы), без масштаба сверх `--scale`; код, терминал и логи - растровый 6x11 редактора (ADR-0004). Поставка конкретного font требует подходящей лицензии и кириллицы.
