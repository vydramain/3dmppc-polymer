...
- **ARC-05 MUST:** отсутствие tools блокирует связанные команды с объяснением, но не открытие и редактирование файлов.
...
- **LAY-05 MUST:** Game сохраняет aspect ratio. По умолчанию integer scale и nearest-neighbor; Fit явно обозначен и не изменяет логическое разрешение консоли.
...
- **TXT-01 MUST:** нормальный немодальный режим по умолчанию: selection, clipboard, Undo/Redo, Save/Save All, find/replace, go to line, indentation, line numbers, UTF-8 и кириллица. Клавиатурное управление не требует знания Vim.
- **TXT-02 MUST:** подсветка Lua, C/C++, TOML; тёмная тема кода в духе Catppuccin Mocha согласована с nvim-deck, независимо от оливкового chrome.
- **TXT-03 MUST:** настройки syntax theme, fonts, keymap и language services разделены. Менять keymap не должно менять тему или indentation.
- **TXT-04 MUST:** configurable tabs/spaces per language, tab width, wrap, ruler, relative/absolute line numbers и indent guides. Начальный профиль C++: настоящие tabs шириной 4, ruler 128, без принудительного wrap; для остальных языков профиль настраивается отдельно.
- **TXT-05 MUST:** поддержать diagnostics/completion/hover/go-to-definition через language-service adapter для C++ и Lua. Серверы могут быть внешними; при их отсутствии базовое редактирование работает, причина отсутствия language features видна.
- **TXT-06 MUST:** Open in External Editor открывает выбранный файл и позицию в настроенном nvim/другом редакторе. Возвращение не теряет внешние изменения.
- **TXT-07 SHOULD:** Vim keymap как опция. Встраивание полного Neovim не требуется для MVP и не подменяет обычный режим.
