-- Fixture: a well-formed entry chunk whose asset_changed(name) writes
-- nothing and returns false with a reason. Proves error=asset_refused,
-- effects=0, and that the old data is left in place.
local M = {}

local state

function M.attach(s)
	state = s
	if state.cv_raw then
		state.cv = pdk.cast("rv_cv*", state.cv_raw)
	end
	return true
end

function M.disc_initialize(cv_, ca_, cio_)
	state.cv_raw = cv_
	state.cv = pdk.cast("rv_cv*", cv_)
end

function M.asset_changed(name)
	return false, "asset_refuses: this chunk never owns anything, on purpose"
end

function M.frame_update(dt)
	return false
end

function M.frame_render()
	pdk.cv_frame_configure(state.cv, 0, pdk.new("rv_color", { 60, 60, 60 }))
end

function M.disc_shutdown() end

return M
