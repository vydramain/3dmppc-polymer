...
// Ревизия: stage 8 — геометрия в rv_primitive: отсечение, культинг, ключ глубины.
// ──────────────────────────────────────────────────────────────────────────────
#pragma once
...
// The bridge: 3D geometry in, rv_primitive out. This is the file that makes the
// rest of sdk/ worth having, because the console's contract stops at int16 screen
// coordinates and a depth SORT KEY — every step between a world-space triangle
// and those numbers is the disc's own work.
//
...
// vertex far off-screen is legal (rv_vertex is signed precisely so a clipped
// corner can be expressed), but a wrapped coordinate would teleport it to the
// opposite edge and drag the whole polygon with it.
...
//
//     area2 = cross(b - a, c - a)      // the 2D cross of sdk/rv_math.hpp
//
...
// addr_palette, mapping) — sdk/ transforms geometry, it does not decide how a
...
inline bool rv_xform_polygon(const rv_xform_conf& conf, const rv_xform_vertex* vertexes, int count,
    ...
    for (int i = count; i < 4; ++i) polygon.vertexes[i] = rv_vertex{};
    return true;
}
