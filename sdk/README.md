...
do; `sdk/` is the arithmetic you would otherwise write again in every game.
...
| [`mppcdiscs/`](../mppcdiscs/) | the **discs** — games | `pdk/` + (optionally) `sdk/` |
| `sdk/` | **conveniences for discs**, written *on top of* the contract | discs only, and only if they want to |

`sdk/` depends on `pdk/` and the standard library. Nothing else — no `src/`, no
SDL. The CMake target is `INTERFACE` with `sdk/include` on the path, and only
disc targets link it.

## Why the console does not link it

Because `sdk/` is **not part of the contract**. The console implements `pdk/`;
...
The direction is one-way and worth stating plainly: `sdk/` is built *over* the
...
GTE was a coprocessor the programmer drove by hand. `sdk/` is that hand.

## Why the parsers do not read files

**Nothing in `sdk/` opens a file.** `rv_obj_parse` takes `const void* data,
...
authoring** job (`tools/`), and what ships on a disc is texels the console can
upload as-is.
...
| --- | --- |
| `sdk/rv_math.hpp` | `rv_vec2/3/4`, `rv_mat4`; dot, cross, length, normalize; translate / rotate / scale; matrix and matrix-vector products |
| `sdk/rv_camera.hpp` | `rv_look_at`, `rv_perspective` (both left-handed), the `rv_camera` bundle, world → clip |
| `sdk/rv_xform.hpp` | geometry → `rv_primitive`: near-plane rejection, perspective divide, screen mapping with `int16` saturation, screen-space back-face culling, the ordering-table depth key |
| `sdk/rv_obj.hpp` | Wavefront `.obj` from a memory buffer (`v` / `vt` / `vn` / `f`, fan triangulation, negative indices) |
| `sdk/rv_color.hpp` | HSV→RGB, lerp / scale / modulate / add, Lambert shading into vertex colours |
