...
uint64_t rv_pccio_sdl3::iport_abilities(int64_t port)
{
...
rv_imouse rv_pccio_sdl3::imouse()
{
...
rv_istate rv_pccio_sdl3::iport_state(int64_t port)
{
    if (!port_in_range(port)) {
        return rv_istate{};
    }
...
int64_t rv_pccio_sdl3::ohaptic(int64_t port, rv_oheffect effect)
{
    if (!port_in_range(port)) {
