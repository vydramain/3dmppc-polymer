...
bool rv_pccl_luajit::bootstrap_pdk()
{
    ...
    // rv_pdklib::RV_PDKLIB_LUA_HELPERS_SRC (pdklib/rv_dscript/rv_dscript_lua.hpp)
    // is pdklib's, not this console's: the resolve helper and the two
    // primitive constructors it adds to `pdk` are the disc author's toolbox,
    ...
    lua_pushvalue(L_, pdk_idx);
    if (lua_pcall(L_, 1, 0, 0) != 0) {
        return fail_err("pdk lua helpers chunk (run)");
    }
