...
int16_t non_negative(int16_t value)
{
    return value < 0 ? static_cast<int16_t>(0) : value;
}
...
{
    if (phase_ == rv_pcvoice_phase::idle) {
        return;
    }
...
{
    if (phase_ == rv_pcvoice_phase::idle || !out) {
        return;
    }
