...
// Revision: stage 10 — disc loader implementation: manifest parsing, extracting the
// code to a temporary file, dlopen, the handshake and teardown in reverse order.
// ──────────────────────────────────────────────────────────────────────────────
...
// size below comes out of a file that may have been downloaded from anywhere, so
// none of them may become a malloc argument unchecked: a two-line header claiming
// a four-gigabyte manifest must cost a log line, not the machine's memory.
...
// Quoted: everything between the quotes, verbatim (a `#` inside quotes is data).
// Bare: everything up to a `#` comment, trimmed. Deliberately forgiving — the
// console reads four keys and has no business failing a manifest over the ones
// it ignores.
std::string manifest_value(std::string_view raw) {
    ...
    if (raw.size() >= 2 && (raw.front() == '"' || raw.front() == '\'')) {
        ...
        if (end != std::string_view::npos) return std::string(raw.substr(1, end - 1));
...
std::string read_whole_entry(const rv_zipreader& zip, const char* name, int64_t max_size,
                             std::vector<unsigned char>& out) {
    const int64_t size = zip.size(name);
    if (size < 0) return "no such entry in the archive";
    if (size > max_size)
    ...
    if (nread != size) return "short read";
    return std::string();
}
...
// 0700 total, this user and nobody else. A group- or world-writable staging file
// would be a way to swap the disc's code out between this write and the dlopen
// a few lines later.
...
bool rv_pcmanifest_parse(const std::string& text, rv_pcdiscinfo& out) {
    ...
    // (pdk/tools/mppcburner/rv_manifest.hpp), where a human is watching and a typo
    // deserves a line number. Here the manifest is untrusted input and the
    ...
    while (!rest.empty()) {
        ...
        if (line.front() == '[') {
            ...
            if (close == std::string_view::npos) continue;  // garbage; not our problem
        ...
        if (key == "id") {
            ...
            if (!value.empty()) out.entry = value;
        }
    }
...
rv_pcloader::~rv_pcloader() { unload(); }
...
int64_t rv_pcloader::load(const char* archive_path) {
    ...
    if (!zip.open(archive_path, zip_error)) {
        RV_LOG_ERR("pcloader", "'{}' is not a readable .mppcdisc archive: {}",
                   rv_log_escape(archive_path), rv_log_escape(zip_error.c_str(), 160));
        return RV_ERR_IO;
    }
    ...
    if (!why.empty()) {
        RV_LOG_ERR("pcloader", "'{}' carries no usable '{}': {}", rv_log_escape(archive_path),
                   kManifestEntry, why);
        return RV_ERR_NOENT;
    }
    ...
    if (!why.empty()) {
        RV_LOG_ERR("pcloader", "disc '{}' names its code entry '{}', which is unusable: {}",
                   rv_log_escape(info_.id.c_str()), rv_log_escape(info_.entry.c_str()), why);
        return RV_ERR_NOENT;
    }
    ...
    if (handle_ == nullptr) {
        ...
        RV_LOG_ERR("pcloader", "dlopen of disc '{}' failed: {}", rv_log_escape(info_.id.c_str()),
...
void rv_pcloader::unload() {
    if (disc_ != nullptr) {
        ...
        if (destroy_ != nullptr) destroy_(disc_);
    }
    disc_ = nullptr;
    destroy_ = nullptr;
    initialized_ = false;
    ...
    if (!temp_path_.empty()) {
        ...
        if (::unlink(temp_path_.c_str()) != 0 && errno != ENOENT) {
            RV_LOG_WARN("pcloader", "could not remove the staging file '{}': {}", temp_path_,
                        std::strerror(errno));
        }
        temp_path_.clear();
    }
