...
class rv_pcpool {
   public:
    ...
    int64_t free(int64_t addr) {
        ...
        if (at < 0) return rv_pdk::RV_ERR_INVAL;

        const size_t index = static_cast<size_t>(at);
        if (!blocks_[index].used || blocks_[index].reserved) return rv_pdk::RV_ERR_INVAL;
    ...
    int64_t write(int64_t addr, const void* data, int64_t bytes) {
        ...
        if (!block) return rv_pdk::RV_ERR_INVAL;

        if (bytes < 0 || bytes > block->size) return rv_pdk::RV_ERR_INVAL;
