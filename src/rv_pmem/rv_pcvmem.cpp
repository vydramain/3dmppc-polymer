...
rv_pcvmem::rv_pcvmem(int64_t reserve_bytes)
{
    if (reserve_bytes <= 0) {
        return;
    }
...
bool rv_pcvmem::valid() const
{
    return valid_;
}

int64_t rv_pcvmem::reserved() const
{
    return reserved_;
}

int64_t rv_pcvmem::ensured() const
{
    return ensured_;
}

uint8_t *rv_pcvmem::base() const
{
    return base_;
}

int64_t rv_pcvmem::ensure(int64_t bytes)
{
    ...
    const int64_t delta = clamped_target - ensured_;
    uint8_t *region = base_ + ensured_;

    if (mprotect(region, static_cast<size_t>(delta), PROT_READ | PROT_WRITE) != 0) {
        RV_LOG_ERR("pcvmem", "mprotect of {} bytes failed: {}", delta, strerror(errno));
        return RV_ERR_NOMEM;
    }
